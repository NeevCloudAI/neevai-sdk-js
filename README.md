...
### Sandbox data-plane (files)

Operations that act inside a running sandbox are reached directly on the sandbox handle (the sandbox must be Ready with a data-plane address):
...
Data-plane calls are **not** retried automatically (a retried `write` could run twice) — handle retries yourself if needed.
