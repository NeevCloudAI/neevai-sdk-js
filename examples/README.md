...
| [`snapshot-fork.ts`](./snapshot-fork.ts) | Create → write a file → snapshot → fork from live state → fork from snapshot → restore in place; reports PASS/FAIL on whether the file survives each path | `npx tsx examples/snapshot-fork.ts` |
